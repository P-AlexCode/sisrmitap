...
try {
    $sql = "SELECT o.id, o.folio, o.fecha_salida, o.fecha_limite_devolucion, o.estado, o.edificios_destino, 
                   p.nombres AS rec_nom, p.apellidos AS rec_ape, 
            ...
            WHERE o.tipo_operacion = 'prestamo'
            ORDER BY o.estado ASC, o.fecha_salida DESC"; // Los pendientes salen arriba
    $stmt = $db->prepare($sql);
    $stmt->execute();
...
// 3. Usuario actual (Para auto-seleccionar Staff)
...
// 5. Productos DEVOLUTIVOS en JSON
$productos_raw = $db->query("
    ...
    WHERE p.estado = 1 AND p.tipo_material = 'devolutivo'
")->fetchAll();
...
<style>
    ...
    .adder-bar {
        background: rgba(13, 202, 253, 0.05);
        border: 1px solid rgba(13, 202, 253, 0.2);
        border-radius: 12px;
        padding: 15px;
    }
</style>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="fw-bold mb-0" style="color: var(--text-main); letter-spacing: -0.5px;">Control de Préstamos</h3>
        <p class="mb-0" style="color: var(--text-muted);">Herramientas, equipos o material que debe ser devuelto al
            almacén.</p>
    </div>
    <button class="btn shadow-sm px-4 py-2" data-bs-toggle="modal" data-bs-target="#modalNuevoPrestamo"
        style="background: var(--accent-color); color: #fff; border: none; border-radius: 12px; font-weight: 600;">
        <i class="bi bi-arrow-left-right me-2"></i> Registrar Préstamo
    </button>
</div>
...
<div class="card border-0 mb-4 shadow-sm"
    ...
    <div class="card-body p-4">
        <div class="table-responsive">
            <table id="tablaPrestamos" class="table table-hover align-middle w-100">
                <thead>
                    <tr style="border-bottom: 2px solid var(--glass-border-panel);">
                        <th class="text-muted" style="font-size: 0.8rem;">FOLIO</th>
                        <th class="text-muted" style="font-size: 0.8rem;">FECHAS (PRESTÓ / LÍMITE)</th>
                        <th class="text-muted" style="font-size: 0.8rem;">SOLICITANTE</th>
                        <th class="text-muted" style="font-size: 0.8rem;">EDIFICIOS</th>
                ...
                <tbody>
                    <?php foreach ($prestamos as $p):
                        $edifs_ids = json_decode($p['edificios_destino'], true) ?: [];
                        $nombres_edifs = [];
                        ...
                        // Lógica de alerta por si está vencido
                        $vencido = false;
                        if ($p['estado'] == 'pendiente_devolucion' && strtotime($p['fecha_limite_devolucion']) < time()) {
                            $vencido = true;
                        }
                        ?>
                        <tr>
                            ...
                            <td>
                                <div class="fw-medium text-success"><i class="bi bi-box-arrow-up-right me-1"></i>
                                    <?= date('d/m/Y', strtotime($p['fecha_salida'])) ?>
                                </div>
                                <div class="small fw-bold <?= $vencido ? 'text-danger' : 'text-warning' ?> mt-1"><i
                                        class="bi bi-clock-history me-1"></i>Vence:
                                    <?= date('d/m/Y h:i A', strtotime($p['fecha_limite_devolucion'])) ?>
                                </div>
                            </td>
                            <td>
                                <div class="fw-bold"><i class="bi bi-person me-1 opacity-75"></i>
                                    <?= htmlspecialchars($p['rec_nom'] . ' ' . $p['rec_ape']) ?>
                                </div>
                            </td>
                            <td>
                                <span class="small opacity-75"><i class="bi bi-buildings me-1"></i>
                                    <?= htmlspecialchars($txt_edificios) ?>
                                </span>
                            </td>
                            <td class="text-center">
                                <?php if ($p['estado'] == 'devuelto_total'): ?>
                                    <span class="badge bg-success bg-opacity-25 text-success border border-success"><i
                                            class="bi bi-check-all me-1"></i>Entregado a Almacén</span>
                                <?php elseif ($vencido): ?>
                                    <span class="badge bg-danger bg-opacity-25 text-danger border border-danger"><i
                                            class="bi bi-exclamation-triangle-fill me-1"></i>Vencido / Con Retraso</span>
                                <?php else: ?>
                                    <span class="badge bg-warning bg-opacity-25 text-warning border border-warning"><i
                                            class="bi bi-hourglass-split me-1"></i>En uso (Pendiente)</span>
                                <?php endif; ?>
                            </td>
                            <td class="text-center">
                                <?php if ($p['estado'] == 'pendiente_devolucion'): ?>
                                    <button class="btn btn-sm btn-outline-success rounded-pill fw-bold border-2"
                                        title="Recibir Todo" onclick="devolverMaterial(<?= $p['id'] ?>, '<?= $p['folio'] ?>')">
                                        <i class="bi bi-arrow-return-left me-1"></i> Recibir Material
                                    </button>
                                <?php else: ?>
                                    <button class="btn btn-sm btn-outline-secondary border-0" disabled><i
                                            class="bi bi-archive-fill fs-5"></i></button>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<form id="formDevolucion" action="<?= BASE_URL ?>views/prestamos/procesar.php" method="POST" style="display: none;">
    <input type="hidden" name="accion" value="devolver_todo">
    <input type="hidden" name="id_operacion" id="dev_id_operacion">
</form>

<div class="modal fade" id="modalNuevoPrestamo" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <form action="<?= BASE_URL ?>views/prestamos/procesar.php" method="POST" id="formPrestamo"
            ...
            <div class="modal-header border-bottom border-secondary border-opacity-25 p-4 flex-shrink-0">
                <h4 class="modal-title fw-bold" style="color: var(--text-main);"><i
                        class="bi bi-arrow-left-right text-primary me-2"></i>Vale de Préstamo de Equipo</h4>
            ...
            <div class="modal-body p-4 p-md-5">
                ...
                <div class="salida-header mb-4">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label text-muted small fw-bold">Personal que Solicita</label>
                        ...
                        <div class="col-md-4">
                            <label class="form-label text-muted small fw-bold"><i
                                    class="bi bi-shield-lock-fill me-1"></i> Lo Autoriza/Entrega</label>
                            <select name="personal_entrega_id" class="form-select border-primary" required
                                ...
                                <?php else: ?>
                                    ...
                                    <?php foreach ($staff_almacen as $staff): ?>
                                        <option value="<?= $staff['id'] ?>" <?= ($staff['id'] == $mi_personal_id) ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($staff['nombres'] . ' ' . $staff['apellidos']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label text-muted small fw-bold text-primary"><i
                                    class="bi bi-calendar-event me-1"></i> Fecha/Hora de Salida</label>
                            <input type="datetime-local" name="fecha_salida" class="form-control border-primary"
                                required value="<?= date('Y-m-d\TH:i') ?>"
                                style="background: rgba(255,255,255,0.1); color: var(--text-main);">
                        </div>
                        <div class="col-md-6 mt-3">
                            <label class="form-label text-muted small fw-bold text-danger"><i
                                    class="bi bi-alarm-fill me-1"></i> Fecha/Hora LÍMITE para Devolver</label>
                            <input type="datetime-local" name="fecha_limite_devolucion"
                ...
                <h6 class="fw-bold mb-3" style="color: var(--accent-color);"><i class="bi bi-laptop me-2"></i>Equipos a
                    Prestar</h6>

                <div class="adder-bar mb-4 shadow-sm">
                    <div class="row align-items-end g-2">
                        <div class="col-md-5">
                            <label class="form-label small fw-bold text-primary mb-1">Buscar Equipo (Solo
                                Devolutivos)</label>
                        ...
                        <div class="col-md-3">
                            <label class="form-label small fw-bold text-primary mb-1">Unidad</label>
                            <select id="adder_unit" class="form-select"
                                style="background: rgba(255,255,255,0.9); border-color: rgba(13, 202, 253, 0.5);">
                                <option value="">---</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            <button type="button" class="btn btn-info text-white w-100 fw-bold shadow-sm"
                                onclick="añadirFilaDesdeAdder()">
                                <i class="bi bi-plus-lg me-1"></i> Bajar
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive mb-3"
                    ...
                    <table class="table table-borderless align-middle mb-0">
                        <thead
                            ...
                            <tr>
                                <th class="text-muted small" width="45%">EQUIPO PRESTADO</th>
                        ...
                        <tbody id="contenedor_filas_salida">
                            <tr id="filaVacia">
                                <td colspan="4" class="text-center text-muted py-5 fst-italic">El vale de préstamo está
                                    vacío.</td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="mt-4 border-top border-secondary border-opacity-25 pt-4">
                    <label class="form-label text-muted small fw-bold">Motivo del Préstamo / Estado del Equipo</label>
                    <textarea name="observaciones" class="form-control" rows="2"
                        placeholder="Ej. Se presta proyector EPSON para evento en el Edificio A. Tiene un ligero golpe en la carcasa pero funciona bien."
            ...
            <div
                ...
                <span class="text-muted small"><i class="bi bi-shield-check text-success me-1"></i> El equipo quedará
                    bajo resguardo del solicitante.</span>
                <div>
                    ...
                    <button type="submit" class="btn btn-lg rounded-pill px-5 fw-bold"
                        ...
                        <i class="bi bi-file-earmark-check me-2"></i> Autorizar Préstamo
                    </button>
                </div>
            </div>
        </form>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#modalNuevoPrestamo').on('shown.bs.modal', function () {
            $('.select2-modal').select2({ dropdownParent: $('#modalNuevoPrestamo'), theme: "bootstrap-5" });
            $('.select2-modal-multi').select2({ dropdownParent: $('#modalNuevoPrestamo'), theme: "bootstrap-5", placeholder: "Seleccione uno o varios..." });
            ...
            adderSelect.select2({ dropdownParent: $('#modalNuevoPrestamo'), theme: "bootstrap-5" });
        });

        $('#formPrestamo').on('submit', function (e) {
            let filas = $('#contenedor_filas_salida .fila-salida').length;
            if (filas === 0) { e.preventDefault(); alert('Agrega al menos un equipo al vale.'); return; }
            ...
            for (let id in consumosAcumulados) {
                if (datosProdSalida[id] && consumosAcumulados[id] > datosProdSalida[id].stock) {
                    hayError = true;
                    msgError = `¡Stock insuficiente!\nEl equipo "${datosProdSalida[id].nombre}" solo tiene ${datosProdSalida[id].stock} disponibles en almacén.`;
                    break;
                }
            }
    ...
    function actualizarAdderUnidades() {
        ...
        if (prodId && datosProdSalida[prodId]) {
            ...
            if (p.stock <= 0) { alert('Equipo actualmente prestado o sin stock.'); }

            p.unidades.forEach((u, index) => { selUnit.append(new Option(u.nombre, u.factor, false, index === 0)); });
    ...
    function añadirFilaDesdeAdder() {
        ...
        if (!prodId) { alert("Selecciona equipo primero."); return; }
        ...
        tr.innerHTML = `
            ...
            <td class="text-center align-middle">
                <span class="badge bg-info text-dark" style="background: rgba(13,202,253,0.2)!important;">${unitText}</span>
            ...
            <td class="text-end align-middle">
                <button type="button" class="btn btn-sm btn-outline-danger border-0 rounded-circle" onclick="quitarFila(this)" title="Quitar">
    ...
    function quitarFila(btnElement) {
        ...
        if (tbody.children.length === 0) {
            tbody.innerHTML = '<tr id="filaVacia"><td colspan="4" class="text-center text-muted py-5 fst-italic">El vale de préstamo está vacío.</td></tr>';
        }
    }

    // SWEETALERT PARA DEVOLVER TODO
    function devolverMaterial(id, folio) {
        const style = getComputedStyle(document.body);
        Swal.fire({
            title: '¿Recibir Equipos?',
            text: `¿Deseas marcar el vale ${folio} como ENTREGADO y devolver los equipos al inventario?`,
            icon: 'info',
            ...
            if (result.isConfirmed) {
                document.getElementById('dev_id_operacion').value = id;
